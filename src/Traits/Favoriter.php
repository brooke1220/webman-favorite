...
trait Favoriter
{
    ...
    {
        /* @var \Brooke1220\WebmanFavorite\Traits\Favoriteable|Model $object */
        if (! $this->hasFavorited($object)) {
            $favorite = Container::get(config('plugin.brooke1220.webman-favorite.app.favorite_model'));
            $favorite->{config('plugin.brooke1220.webman-favorite.app.user_foreign_key')} = $this->getKey();
    ...
    {
        /* @var \Brooke1220\WebmanFavorite\Traits\Favoriteable $object */
        $relation = $object->favorites()
            ->where('favoriteable_id', $object->getKey())
            ->where('favoriteable_type', $object->getMorphClass())
            ->where(config('plugin.brooke1220.webman-favorite.app.user_foreign_key'), $this->getKey())
            ->first();
    ...
    {
        return $this->hasMany(config('plugin.brooke1220.webman-favorite.app.favorite_model'), config('plugin.brooke1220.webman-favorite.app.user_foreign_key'), $this->getKeyName());
    }
    ...
    {
        return \support\Container::get($model)->whereHas(
            'favoriters',
            function ($q) {
                return $q->where(config('plugin.brooke1220.webman-favorite.app.user_foreign_key'), $this->getKey());
            }
        );
    }
}
